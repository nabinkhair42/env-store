...
export function useEnvVariables(
...
): UseEnvVariablesState & UseEnvVariablesActions {
  const [variables, setVariablesState] =
    useState<EnvVariable[]>(initialVariables);
  ...
  const importFromText = useCallback((content: string) => {
    try {
      ...
      toast.success(parsed.length + ' variables imported successfully');
    } catch (error) {
  ...
  const exportToFile = useCallback(
    (filename: string) => {
      try {
        ...
        toast.success('Environment variables exported to ' + filename);
      } catch (error) {
