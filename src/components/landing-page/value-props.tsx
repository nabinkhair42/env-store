...
export default function ValueProps() {
  return (
    <section className="mx-auto w-full max-w-4xl px-6 py-20">
      ...
      <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-3 divide-y sm:divide-y-0 divide-border">
        {features.map((item, i) => (
          <div
            key={i}
            className={`p-6 ${i % 3 !== 0 ? 'lg:border-l lg:border-border' : ''} ${i % 2 !== 0 ? 'sm:max-lg:border-l sm:max-lg:border-border' : ''} ${i >= 3 ? 'lg:border-t lg:border-border' : ''} ${i >= 2 ? 'sm:max-lg:border-t sm:max-lg:border-border' : ''}`}
          >
            <div className="mb-4 inline-flex size-10 items-center justify-center rounded-lg border border-border text-muted-foreground">
